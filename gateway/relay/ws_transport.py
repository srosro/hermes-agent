...
class WebSocketRelayTransport:
    ...
    async def get_chat_info(
        ...
        # Multi-platform relays: route through the OutboundFrame platform seam
        # (frame-level platform + matching bot id) so the connector answers
        # from the right lane instead of the primary's.
        result = await self._request_response(
            {"op": "get_chat_info", "chat_id": chat_id},
            frame_type="outbound",
            platform=platform,
        )
