...
**群组主频道：** 在群组中运行 `/sethome` 会记录该会话的身份，切换到该主频道时会保留其群组（以及在按参与者作用域下的参与者）session 键——下一条自然回复会继续切换过来的会话。只有从未记录 `chat_type` 的旧版或手动环境配置的主频道才会回退到推断；在目标会话中重新运行 `/sethome` 即可记录。
