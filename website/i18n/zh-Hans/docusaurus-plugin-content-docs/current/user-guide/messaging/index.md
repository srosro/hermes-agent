...
第一次在任意平台向繁忙的 agent 发送消息时，Hermes 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示会为每个路由配置文件触发一次——由该配置文件 `config.yaml` 中的 `onboarding.seen.busy_input_prompt` 标志锁定。从对应配置文件中删除该键可再次看到提示。
